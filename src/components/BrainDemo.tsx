    ...
    <div className="card p-3 sm:p-5">
      ...
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          ...
          <h3 className="text-[14px] sm:text-[16px] font-medium text-white">See Cortex in Action</h3>
        </div>
        <div className="flex gap-1 p-0.5 rounded-lg bg-white/[0.03] border border-[var(--border)]">
          {[{ id: "nt" as const, l: "NT" }, { id: "nd" as const, l: "ND" }, { id: "compare" as const, l: "Both" }].map((m) => (
            <button key={m.id} onClick={() => setMode(m.id)}
              className={`text-[10px] sm:text-[11px] px-2.5 py-1 rounded transition ${mode === m.id ? "bg-white/10 text-white" : "text-[var(--muted)]"}`}>
              {m.l}
            </button>
          ))}
        </div>
      ...
      {/* Videos: stimulus + brains in a single row on desktop */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-3">
        {/* Stimulus */}
        <div>
          <div className="text-[9px] text-[var(--muted)] font-medium mb-1">Stimulus</div>
        ...
        {(mode === "nt" || mode === "compare") && (
          <div>
            <div className="text-[9px] text-[var(--accent)] font-medium mb-1">Neurotypical</div>
            <div className="rounded-md overflow-hidden bg-black aspect-video">
        ...
        {(mode === "nd" || mode === "compare") && (
          <div>
            <div className="text-[9px] text-[var(--accent2)] font-medium mb-1">Neurodiverse</div>
            <div className="rounded-md overflow-hidden bg-black aspect-video">
      ...
      {/* Current event + timeline combined */}
      ...
      <div className="flex items-center justify-center gap-2">
        <button onClick={() => { syncSeek(0); syncPlay(); }} className="text-[11px] px-3 py-1.5 rounded-full border border-[var(--border)] text-[var(--muted)] hover:text-white transition">Restart</button>
        <button onClick={playing ? syncPause : syncPlay} className="text-[11px] px-5 py-1.5 rounded-full bg-white text-[#050507] font-medium hover:bg-white/90 transition">
          {playing ? "Pause" : "Play"}
        </button>
      </div>
    </div>
